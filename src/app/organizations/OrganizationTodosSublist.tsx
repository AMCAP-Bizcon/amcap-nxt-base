...
interface OrganizationTodosSublistProps {
    linkedIds: number[];
    availableTodos: Todo[];
...
export interface OrganizationTodosSublistRef {
    saveIfUnsaved: () => Promise<void>;
}

export const OrganizationTodosSublist = forwardRef<OrganizationTodosSublistRef, OrganizationTodosSublistProps>(({ linkedIds, availableTodos, allTodosMap, readOnly = false, onLinksChanged, onClickTodo, onModeChange }, ref) => {
    ...
    const localTodos = localLinkedIds.map(id => allTodosMap.get(id)!).filter(Boolean)
        ...
        <div className="flex flex-col gap-3">
            {!readOnly && (
                <ResponsiveToolbar>
                    {mode === 'idle' ? (
                        <>
                            <ToolbarButton variant="outline" onClick={() => setMode('creating')} className="h-9 text-violet-600 hover:text-violet-700 hover:bg-violet-50 hover:shadow-glow-violet-sm" icon={<PlusCircle />} label="Link Todo" />
            ...
            <ul className={cn("space-y-2 mt-2 transition-all duration-300", mode !== 'idle' ? `p-2 border rounded-md ${modeStyles[mode]} bg-background/50` : "")}>
                {mode === 'creating' && (
                    <li className="p-0 border border-primary/50 rounded-md bg-card flex flex-col overflow-hidden shadow-sm">
                        <Command className="w-full bg-transparent p-0">
                            <CommandInput
                                placeholder={`Search to link a todo...`}
                                value={searchText}
                            ...
                            <CommandList>
                                <CommandEmpty className="py-2 text-center text-sm text-muted-foreground">
                                    No matched todo found.
                                </CommandEmpty>
                                <CommandGroup>
                                    {availableTodos.filter(p => !localLinkedIds.includes(p.id)).map((p) => (
                                        <CommandItem
                                            key={p.id}
                                            value={p.text}
                                            onSelect={() => {
                ...
                {localTodos.length === 0 && mode !== 'creating' && (
                    <p className="text-sm text-muted-foreground p-2 italic">No todos linked.</p>
                )}
                {localTodos.map(p => (
                    <li
                        key={p.id}
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-0.5 w-full">
                            <span
                                className={cn(
                                    "font-semibold",
                                    onClickTodo && "hover:text-primary transition-colors cursor-pointer"
                                )}
                                onClick={() => onClickTodo?.(p.id)}
                            >
                                {p.text}
                            </span>
                        </div>
...
OrganizationTodosSublist.displayName = 'OrganizationTodosSublist'
