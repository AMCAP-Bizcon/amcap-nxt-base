...
import { type Profile, type Organization } from '@/db/schema'
...
interface RoleAssignmentsSublistProps {
    assignments: RoleAssignment[];
    allProfilesMap: Map<string, Profile>;
    allOrganizationsMap: Map<number, Organization>;
    readOnly?: boolean;
    onAssignmentsChanged: (newAssignments: RoleAssignment[]) => void;
    onModeChange?: (mode: 'idle' | 'creating' | 'delete') => void;
}

export interface RoleAssignmentsSublistRef {
    saveIfUnsaved: () => Promise<void>;
}

export const RoleAssignmentsSublist = forwardRef<RoleAssignmentsSublistRef, RoleAssignmentsSublistProps>(({ assignments, allProfilesMap, allOrganizationsMap, readOnly = false, onAssignmentsChanged, onModeChange }, ref) => {
        ...
        try {
            if (mode === 'delete') {
                ...
                if (newUser && newOrg) {
                    ...
                    // Check if exists
                    const exists = localAssignments.some(a => a.userId === newUser && a.organizationId === orgIdNum)
                    if (!exists) {
                        const newAsgmts = [...localAssignments, { userId: newUser, organizationId: orgIdNum }]
                        setLocalAssignments(newAsgmts)
                        onAssignmentsChanged(newAsgmts)
                    }
                }
        ...
        <div className="flex flex-col gap-3">
            ...
            <ul className={cn("space-y-2 mt-2 transition-all duration-300", mode !== 'idle' ? `p-2 border rounded-md ${modeStyles[mode]} bg-background/50` : "")}>
                ...
                {localAssignments.length === 0 && mode !== 'creating' && (
                    <p className="text-sm text-muted-foreground p-2 italic">No users assigned to this role.</p>
                )}
...
RoleAssignmentsSublist.displayName = 'RoleAssignmentsSublist'
