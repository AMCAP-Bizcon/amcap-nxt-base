...
import { type AccessRule, AVAILABLE_APP_TABLES, type AppTableName } from '@/db/schema'
...
export function RoleAccessRulesSublist({ roleId, accessRules, readOnly }: RoleAccessRulesSublistProps) {
    const [rules, setRules] = useState<AccessRule[]>([])
    const [mode, setMode] = useState<'idle' | 'delete' | 'toggle'>('idle')
    ...
    const handleAddRule = async () => {
        ...
        const existingTables = rules.map(r => r.tableName);
        const nextTable = AVAILABLE_APP_TABLES.find(t => !existingTables.includes(t));
        const newTableName = nextTable ? nextTable : AVAILABLE_APP_TABLES[0];

        if (!newTableName) return; // No tables exist
    ...
    const toolbarActions = mode === 'idle' ? (
        !readOnly && (
            <>
                ...
                <ToolbarButton variant="outline" onClick={() => setMode('toggle')} className="h-8 text-amber-600 hover:text-amber-700 hover:bg-amber-50" icon={<Power className="w-4 h-4" />} label="Toggle Active" disabled={rules.length === 0} />
            </>
        )
    ) : (
        <>
            ...
            {mode === 'toggle' && (
                <ToolbarButton variant="outline" onClick={handleToggleActiveSelected} className="h-8 text-amber-600 hover:text-amber-700 hover:bg-amber-50" icon={<Power className="w-4 h-4" />} label="Confirm Toggle" disabled={selectedRuleIds.length === 0} />
            )}
        </>
    )
    ...
    return (
        <div className="flex flex-col gap-3">
            ...
            <ul className="space-y-4">
                {rules.map(rule => (
                    <li key={rule.id} className={`p-4 rounded-lg border flex flex-col gap-3 ${mode !== 'idle' ? 'cursor-pointer hover:border-slate-300 dark:hover:border-slate-600' : ''} ${selectedRuleIds.includes(rule.id) ? 'border-primary ring-1 ring-primary' : 'border-border'}`} onClick={() => {
                        if (mode !== 'idle') {
                            ...
                            <div className="flex items-center gap-2 flex-wrap flex-1">
                                ...
                                <div onClick={e => e.stopPropagation()}>
                                    <Select 
                                        ...
                                        onValueChange={(val) => handleChangeRecord(rule.id, 'tableName', val)}
                                        disabled={readOnly || mode !== 'idle'}
                                    >
                                        <SelectTrigger className="h-8 w-auto min-w-[120px] bg-transparent font-semibold border-none focus:ring-0 text-foreground text-sm outline-none disabled:opacity-50 p-0 pr-2 shadow-none [&>svg]:opacity-50 hover:bg-transparent data-[state=open]:bg-transparent">
                                            <SelectValue />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {AVAILABLE_APP_TABLES.map(t => (
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 ml-6 text-sm">
                            <label className="flex items-center gap-2 cursor-pointer" onClick={e => e.stopPropagation()}>
                                <input type="checkbox" checked={rule.canRead} disabled={readOnly || mode !== 'idle'} onChange={(e) => handleChangeRecord(rule.id, 'canRead', e.target.checked)} className="rounded border-gray-300 text-primary focus:ring-primary h-4 w-4" />
                                Read
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer" onClick={e => e.stopPropagation()}>
                                <input type="checkbox" checked={rule.canCreate} disabled={readOnly || mode !== 'idle'} onChange={(e) => handleChangeRecord(rule.id, 'canCreate', e.target.checked)} className="rounded border-gray-300 text-primary focus:ring-primary h-4 w-4" />
                                Create
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer" onClick={e => e.stopPropagation()}>
                                <input type="checkbox" checked={rule.canUpdate} disabled={readOnly || mode !== 'idle'} onChange={(e) => handleChangeRecord(rule.id, 'canUpdate', e.target.checked)} className="rounded border-gray-300 text-primary focus:ring-primary h-4 w-4" />
                                Update
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer" onClick={e => e.stopPropagation()}>
                                <input type="checkbox" checked={rule.canDelete} disabled={readOnly || mode !== 'idle'} onChange={(e) => handleChangeRecord(rule.id, 'canDelete', e.target.checked)} className="rounded border-gray-300 text-primary focus:ring-primary h-4 w-4" />
                                Delete
                            </label>
                        </div>
                    </li>
