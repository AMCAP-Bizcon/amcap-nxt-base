...
interface UserRelationshipSubListProps {
    title: string;
    linkedIds: string[];
...
export interface UserRelationshipSubListRef {
    saveIfUnsaved: () => Promise<void>;
}

export const UserRelationshipSubList = forwardRef<UserRelationshipSubListRef, UserRelationshipSubListProps>(({ title, linkedIds, availableProfiles, allProfilesMap, readOnly = false, onLinksChanged, onClickUser, onModeChange }, ref) => {
        ...
        <div className="flex flex-col gap-3">
            ...
            <ul className={cn("space-y-2 mt-2 transition-all duration-300", mode !== 'idle' ? `p-2 border rounded-md ${modeStyles[mode]} bg-background/50` : "")}>
                {mode === 'creating' && (
                    <li className="p-0 border border-primary/50 rounded-md bg-card flex flex-col overflow-hidden shadow-sm">
                        <Command className="w-full bg-transparent p-0">
                            <CommandInput
                                placeholder={`Search to link a ${title.toLowerCase()}...`}
                                value={searchText}
                            ...
                            <CommandList>
                                <CommandEmpty className="py-2 text-center text-sm text-muted-foreground">
                                    No matched {title.toLowerCase()} found.
                                </CommandEmpty>
                                <CommandGroup>
                ...
                {localProfiles.length === 0 && mode !== 'creating' && (
                    <p className="text-sm text-muted-foreground p-2 italic">No {title.toLowerCase()} linked.</p>
                )}
                {localProfiles.map(p => (
                    <li
                        key={p.id}
...
UserRelationshipSubList.displayName = 'UserRelationshipSubList'
