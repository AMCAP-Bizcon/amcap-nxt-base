...
interface UserOrganizationsSublistProps {
    linkedIds: number[];
    availableOrganizations: Organization[];
    allOrganizationsMap: Map<number, Organization>;
    readOnly?: boolean;
    onLinksChanged: (newIds: number[]) => void;
    onClickOrganization?: (id: number) => void;
    onModeChange?: (mode: 'idle' | 'creating' | 'delete') => void;
}

export interface UserOrganizationsSublistRef {
    saveIfUnsaved: () => Promise<void>;
}

export const UserOrganizationsSublist = forwardRef<UserOrganizationsSublistRef, UserOrganizationsSublistProps>(({ linkedIds, availableOrganizations, allOrganizationsMap, readOnly = false, onLinksChanged, onClickOrganization, onModeChange }, ref) => {
    ...
    const localOrganizations = localLinkedIds.map(id => allOrganizationsMap.get(id)!).filter(Boolean)
        ...
        <div className="flex flex-col gap-3">
            {!readOnly && (
                <ResponsiveToolbar>
                    {mode === 'idle' ? (
                        <>
                            <ToolbarButton variant="outline" onClick={() => setMode('creating')} className="h-9 text-violet-600 hover:text-violet-700 hover:bg-violet-50 hover:shadow-glow-violet-sm" icon={<PlusCircle />} label="Link Organization" />
            ...
            <ul className={cn("space-y-2 mt-2 transition-all duration-300", mode !== 'idle' ? `p-2 border rounded-md ${modeStyles[mode]} bg-background/50` : "")}>
                        ...
                        <Command className="w-full bg-transparent p-0">
                            ...
                            <CommandList>
                                <CommandEmpty className="py-2 text-center text-sm text-muted-foreground">
                                    No matched organization found.
                                </CommandEmpty>
                                <CommandGroup>
                                    {availableOrganizations.filter(p => !localLinkedIds.includes(p.id)).map((p) => (
                                        <CommandItem
                                            key={p.id}
                                            value={p.name}
                                            onSelect={() => {
                ...
                {localOrganizations.length === 0 && mode !== 'creating' && (
                    <p className="text-sm text-muted-foreground p-2 italic">No organizations linked.</p>
                )}
                {localOrganizations.map(p => (
                    <li
                        key={p.id}
                        className={cn(
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-0.5 w-full">
                            <span
                                className={cn(
                                    "font-semibold",
                                    onClickOrganization && "hover:text-primary transition-colors cursor-pointer"
                                )}
                                onClick={() => onClickOrganization?.(p.id)}
                            >
                                {p.name}
                            </span>
                        </div>
...
UserOrganizationsSublist.displayName = 'UserOrganizationsSublist'
